...
| Indexer + API | Event subscriber and REST/WebSocket service | `api.superhero.com` |
...
| Identity | `AddressLink` | Provider links: preferred `.chain` name, X handle, Nostr key, bio, website |
...
- **Address-link provider wallet** — the backend account that owns the `x`, `nostr`, `bio`, `site`
  and `prefaens` provider namespaces in `AddressLink`. It checks the off-chain proof (an OAuth
  session, a signed Nostr event, AENS ownership), then submits and pays for the `link` call the
  user signed. See [Verifying Your X Account](/identity/x-verification).
...
| Key | Mainnet | Testnet |
| --- | --- | --- |
| `BACKEND_URL` | `https://api.superhero.com` | Not publicly documented |
