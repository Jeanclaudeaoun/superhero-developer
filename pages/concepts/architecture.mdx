...
**The indexer reads the middleware, not the node directly.** Everything upstream of the plugins goes
through `MIDDLEWARE_URL`, so the middleware's own lag is the floor on how fresh any API response can
be.
...
- [superhero-api](https://github.com/superhero-com/superhero-api) — the indexer and API described above.
